...
# GitHub OIDC provider (if not already created in your account)
...
# Permissions for deploy: ECR push, ECS register TD + update service, logs describe
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      { "Effect":"Allow", "Action":["iam:PassRole"], "Resource":[
        aws_iam_role.ecs_task_execution.arn, aws_iam_role.ecs_task.arn
      ]},
      { "Effect":"Allow", "Action":["logs:DescribeLogGroups","logs:CreateLogGroup"], "Resource":"*" }
    ]
  })
}
